...
const users = {
    ...
    DeleteUser: async (req, res) => {
        ...
        conn.query('select Img from Profiles where UserId=?', [UserId], (err, img, result) => {
            if (err) throw err;
            conn.query('DELETE FROM Users where UserId=?', [UserId], (err, result) => {
                ...
                conn.query("DELETE FROM Profiles WHERE UserId=?", [UserId], (err, results) => {
                    ...
                    conn.query("DELETE FROM Users_has_Roles WHERE UserId=?", [UserId], (err, results) => {
                        if (err) {
                            console.log(err);
                            return res.status(303).json({ status: "ok", Message: "Error any code..." });
                        }
                        conn.query("DELETE FROM Permissions WHERE UserId=?", [UserId], (err, results) => {
                            if (err) {
                                console.log(err);
                                return res.status(303).json({ status: "ok", Message: "Error any code..." });
                            }
